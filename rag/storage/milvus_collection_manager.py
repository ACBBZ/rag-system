...
from rag.storage.milvus_schema import build_collection_schema, described_vector_dimension
...
class MilvusCollectionManager:
    ...
    def _validate_collection(self, resource: TenantVectorResource) -> None:
        ...
        if dimension != resource.embedding_dimension:
            raise ValueError(
                "existing Milvus collection embedding dimension does not match resource"
            )
        field_names = {
            field.get("name")
        ...
        missing = _REQUIRED_FIELDS.difference(field_names)
        if missing:
            raise ValueError(
